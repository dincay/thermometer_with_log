...
{ 
  ...
  Wire.send(pointerReg); // Open specified register
...
uint8_t TMP102::readRegister(bool registerNumber){  
  ...
  registerByte[0] = (Wire.receive());	// Read first byte
  registerByte[1] = (Wire.receive());	// Read second byte
...
void TMP102::setConversionRate(uint8_t rate)
{
  ...
  Wire.send(CONFIG_REGISTER); 	// Point to configuration register
  Wire.send(registerByte[0]);  // Write first byte
  Wire.send(registerByte[1]);  // Write second byte
  Wire.endTransmission();  		// Close communication with TMP102
}
...
void TMP102::sleep(void)
{
  ...
  Wire.beginTransmission(_address);
  Wire.send(CONFIG_REGISTER);	// Point to configuration register
  Wire.send(registerByte);     // Write first byte
  Wire.endTransmission(); 	    // Close communication with TMP102
}
...
void TMP102::setLowTempC(float temperature)
{
  ...
  Wire.beginTransmission(_address);
  Wire.send(T_LOW_REGISTER); 	// Point to T_LOW
  Wire.send(registerByte[0]);  // Write first byte
  Wire.send(registerByte[1]);  // Write second byte
  Wire.endTransmission();  		// Close communication with TMP102
}
...
void TMP102::setHighTempC(float temperature)
{
  ...
  Wire.beginTransmission(_address);
  Wire.send(T_HIGH_REGISTER); 	// Point to T_HIGH register
  Wire.send(registerByte[0]);  // Write first byte
  Wire.send(registerByte[1]);  // Write second byte
  Wire.endTransmission();  		// Close communication with TMP102
}
...
void TMP102::setFault(uint8_t faultSetting)
{
  ...
  Wire.beginTransmission(_address);
  Wire.send(CONFIG_REGISTER); 	// Point to configuration register
  Wire.send(registerByte);     // Write byte to register
  Wire.endTransmission();       // Close communication with TMP102
}
